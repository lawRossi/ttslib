...
def compute_pictch(audio, sampling_rate, hop_length, durations=None):
    pitch, t = pw.dio(
        audio.astype(np.float64),
        sampling_rate,
        frame_period=hop_length/sampling_rate*1000,
    )
    pitch = pw.stonemask(audio.astype(np.float64), pitch, t, sampling_rate)
    if np.sum(pitch != 0) <= 1:
        return None
...
def compute_mel_spectrogram_and_energy(audio, stft_cls, durations=None):
    ...
    if mel_spectrogram.shape[1] < sum(durations):
        return None, None
...
def inv_mel_spec(mel, stft_cls, griffin_iters=60):
    mel = torch.stack([mel])
    mel_decompress = stft_cls.spectral_de_normalize(mel)
    mel_decompress = mel_decompress.transpose(1, 2).data.cpu()
    spec_from_mel_scaling = 1000
    spec_from_mel = torch.mm(mel_decompress[0], stft_cls.mel_basis)
    spec_from_mel = spec_from_mel.transpose(0, 1).unsqueeze(0)
    spec_from_mel = spec_from_mel * spec_from_mel_scaling

    audio = griffin_lim(
        torch.autograd.Variable(spec_from_mel[:, :, :-1]), stft_cls.stft_fn, griffin_iters
    )
    audio = audio.squeeze()
    return audio
