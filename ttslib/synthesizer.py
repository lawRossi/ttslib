...
import aukit
from mbmelgan.vocoder import MbMelganVocoder
import numpy as np
import pickle
import pydub
import torch
...
class Synthesizer:
    ...
    def _init_vocoder(self, model_config):
        ...
        elif self.vocoder_model == "mbmelgan":
            self.vocoder = MbMelganVocoder("models/mbmelgan/mb_melgan_0610.pt")
...
if __name__ == "__main__":
    ...
    texts = ["通过设置用户偏好", "广州女大学生登山失联", "我很想你呢，你在干什么？有没有想我？", "今天天气很好你打算做什么？我们一起出去玩吧",
             "你我相识本是一场误会让我们从此别再见", "扶老奶奶过马路是我们都应该做的", "图啥自强自立，不过三十而已",
             "我们的价值观不一样，不能成为好朋友"]

    # for speaker in ["SSB0080", "SSB0005"]:
    ...
    for speaker, reference_key in references:
        for i, text in enumerate(texts):
            synthesizer.synthesize(text, f"demo1/unet_demo_{speaker}{i}.wav", speaker, reference_key)
